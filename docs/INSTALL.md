...
3. Manually update your iTerm profile to use a new font (_Knack Regular Nerd Font Complete_ is added by the installation script) and colorscheme.
...
_Note:_ Cloning this repo probably isn't necessary. You only need to grab the `config/nvim/init.vim` and `config/nvim/plugins.vim` files from this repo and put them in the correct locations on your computer.
...
In order to keep your fingers near the home row, mapping caps lock to escape to get out of different vim modes can be extremely helpful. This is especially useful for laptops that made the escape key "virutal" (_I'm looking at you, Apple_).
